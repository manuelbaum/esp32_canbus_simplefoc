                             ...
                             message_velocity()
{
  ...
  map_mac_to_id = {
      ...
      {std::array<uint8_t, 6>({0x24, 0xec, 0x4a, 0x2d, 0x08, 0xd4}), 3},
      {std::array<uint8_t, 6>({0x24, 0xec, 0x4a, 0x2d, 0x08, 0xc8}), 4},
      {std::array<uint8_t, 6>({0x24, 0xec, 0x4a, 0x2d, 0x08, 0xd0}), 5},
      {std::array<uint8_t, 6>({0x34, 0xb7, 0xda, 0x5a, 0x48, 0xb8}), 6}, // this one still thinks it is 3
  ...
  message_velocity.identifier = MSG_IDS_REL::STATE_VELOCITY + ((id + 1) * 100);
...
void PadmanESP32::init_canbus()
{
  ...
  xTaskCreatePinnedToCore([](void *obj)
      ...
      instance->canbus_polling(); }, "TWAI Receive Task", 4096, this, 1, NULL, core_id);
...
void PadmanESP32::canbus_callback()
{
  ...
  while (twai_receive(&message, pdMS_TO_TICKS(1)) == ESP_OK)
  {
    ...
    if ((message.identifier / 100) - 1 == id)
    { // is this for this joint?
      ...
      case MSG_IDS_REL::TARGET_TORQUE:
        ...
        // joint_target = (float_t)*message.data;
        float new_target_torque;
        memcpy(&new_target_torque, message.data, sizeof(new_target_torque));
        set_tau_d(new_target_torque);
        // printf("Received a new joint target %f", joint_target);
        break;
      }
    }
    else if ((message.identifier / 100) == id)
    { // is it for the previous joint? (we check because we only send our position if the previous joint sent its position)
      switch (message.identifier % 100)
      {
...
void PadmanESP32::send_canbus_velocity()
{
  float velocity = joint_velocity();
  if (velocity != velocity)
  {
    printf("NAN!!!!");
    printf("Joint %i Sending velocity: %f %f %f\n", id, velocity, motor.shaftAngle(), sensor.getAngle());
  }

  memcpy(message_velocity.data, &velocity, sizeof(velocity));
  // Queue message for transmission
  if (twai_transmit(&message_velocity, pdMS_TO_TICKS(10)) == ESP_OK)
  {
    // printf("Message queued for transmission from id %d\n",id);
  }
  else
...
void PadmanESP32::print_id_can_and_mac(){
  printf("ID: %u \tMAC address: %02x:%02x:%02x:%02x:%02x:%02x\n", id, mac8[0], mac8[1], mac8[2], mac8[3], mac8[4], mac8[5]);
}
...
void PadmanESP32::loop()
{
  ...
  case CTRL_POSITION:
    motor.controller = MotionControlType::torque;
    tau = sign * kp * (get_x_d() - joint_position());
    tau += -sign * kd * joint_velocity();
    tau = tau / kT;
    motor.target = tau;
  ...
  unsigned long t_now = millis();
    count_fps_control++;
    if(t_now > t_prev_fps_control + 1000){
      printf("CONTROL CORE: %i FPS: %i\n", xPortGetCoreID(), count_fps_control);
      fflush(stdout);
      t_prev_fps_control = t_now;
      count_fps_control = 0;
    }
...
float PadmanESP32::joint_velocity()
{
  float motor_velocity = motor.shaftVelocity(); // compute off-zero position in motor space (temporarily assuming zero is in the middle of joint limits in motor space)
  float joint_velocity = sign * (motor_velocity / gear_ratio);             // transfer to joint space, first using gear ratio, then offsetting joint_offset which is specified in joint-space
